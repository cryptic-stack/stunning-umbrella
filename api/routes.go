...
func RegisterRoutes(r *gin.Engine, h *handlers.Handler, authMiddleware gin.HandlerFunc, requireRoles func(...string) gin.HandlerFunc) {
	...
	viewer.GET("/api/uploads", h.ListUploads)
	viewer.GET("/frameworks", h.GetFrameworks)
	viewer.GET("/api/frameworks", h.GetFrameworks)
	viewer.GET("/frameworks/:id/versions", h.GetFrameworkVersions)
	viewer.GET("/api/frameworks/:id/versions", h.GetFrameworkVersions)
	...
	viewer.GET("/api/workflow/catalog", h.WorkflowCatalog)
	...
	cisBench := protected.Group("/")
	cisBench.Use(requireRoles("admin"))
	cisBench.GET("/cis-bench/status", h.CISBenchStatus)
	cisBench.POST("/cis-bench/login", h.CISBenchLogin)
	cisBench.POST("/cis-bench/logout", h.CISBenchLogout)
	cisBench.GET("/cis-bench/cookies/export", h.CISBenchExportCookies)
	cisBench.POST("/cis-bench/catalog/refresh", h.CISBenchRefreshCatalog)
	cisBench.POST("/cis-bench/search", h.CISBenchSearch)
	cisBench.POST("/cis-bench/download", h.CISBenchDownload)
	cisBench.GET("/cis-bench/files", h.CISBenchListFiles)
	cisBench.DELETE("/cis-bench/files", h.CISBenchDeleteFiles)
	cisBench.DELETE("/cis-bench/files/:name", h.CISBenchDeleteFile)
	cisBench.GET("/cis-bench/files/:name/download", h.CISBenchDownloadFile)
}
